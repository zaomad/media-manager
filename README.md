...
- [x] 修复了音乐状态保存问题
- [x] 修复了书籍按分类筛选功能
- [x] 完善豆瓣导入功能，支持电影、图书和音乐信息的导入
- [x] 修复了豆瓣导入电影和音乐时的状态选择问题
- [x] 统一电影和音乐的状态选项为"未看/听"、"想看/听"和"已看/听"
- [x] 优化豆瓣搜索结果显示，添加更多媒体信息
- [x] 改进豆瓣搜索结果卡片，显示导演、演员、作者、出版社等信息
...
Media Manager现已集成豆瓣数据获取功能，可以直接从豆瓣导入电影、图书和音乐信息。

### 功能特点

- 支持从豆瓣搜索电影、图书和音乐
- 查看详细的条目信息，包括评分、简介、演员/作者等
- 一键导入到媒体库，自动填充元数据
- 支持添加个人评分、笔记和状态
- 电影显示导演和主演信息
- 图书显示作者和出版社信息
- 音乐显示艺术家和专辑信息
- 统一的状态选项："未看/听"、"想看/听"和"已看/听"

### 使用方法

1. 点击导航栏中的"豆瓣搜索"
2. 输入关键词并选择类型（全部/电影/图书/音乐）
3. 在搜索结果中点击"查看详情"
4. 在详情页面查看完整信息，并可选择添加到媒体库
5. 选择适当的状态、评分，添加个人笔记
6. 点击"导入"按钮，将条目添加到媒体库

### 技术实现

豆瓣集成功能使用Python的requests和BeautifulSoup库实现，通过解析豆瓣网页和API获取数据。该功能模块化设计，便于维护和扩展。搜索结果和详情页面经过优化，提供更丰富的媒体信息。
